/**
 * Created by lichenchen on 2017/11/25.
 */
...
class QueryDataStream extends Writable {
    ...
    _write(chunk, enc, done) {
        ...
        var cpcontentid = content.replace(/(^\s*)|(\s*$)/g, "");
        ...
        checkVideoType(vid).then(function (result) {
                if (result.code == 200) {
                    ...
                    sequelize.query("update Program set videoType=:videoType,videoDesc=:videoDesc,updateTime=:updateTime where cpContentID= :cpContentID and updateTime<=:oldTime", {
                        replacements: {
                            videoType: typeCount,
                            videoDesc: type,
                            cpContentID: cpcontentid,
                            updateTime: new Date(),
                            oldTime:"2017-11-23 16:28:00"
                        },
                        type: sequelize.QueryTypes.UPDATE
                    }).spread(function (results, metadata) {
                        console.log(vid + " update program suceess at " + new Date());
                        done(null, chunk);
